 ...
 * For shorter strings, the single line StringField component is used.
 ...
 */
public class TextField extends TextArea implements IFieldRenderer
{
    ...
    {
        if (getTargetAsTextField(event.target) !== this)
        {
    ...
    {
        const textField:TextField = target as TextField;
        if (textField)
        {
            return textField;
        }
        if (target.parent)
        {
            return getTargetAsTextField(target.parent);
        }
        return null;
    }
